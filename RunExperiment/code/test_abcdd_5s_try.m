...
function [IsQuit, Performance] = test_abcdd_5s
...
%     'endpoint6'
    ...
    for i = 1:numBlocks
        ...
        allList= ceil(imagesPerBlock*0.2+1):1:round(imagesPerBlock*0.8);
        ...
        % first 20% of total trial number. 2 sec should be substituted by how
        % predefined responseWindow
        % targets are never next to each other
        
        
        for j=1:imagesPerBlock
            for k = 1:length(targ)
                if j==targ(k)
    ...
    for theBlock=1:numBlocks
        ...
        for i = 1:imagesPerBlock
            ...
            for theframe = 1:framesPerItem
                ...
                Screen('DrawTexture', window, offScrPtr(whichGender,thepic,theframe),[],CenterRectOnPoint(pixRect_scale, screenRect(3)/2,screenRect(4)/2));
                presStart = GetSecs;
    ...
    elseif length(rsp)>=1
        for i=1:length(rsp)
            for j=0:ceil(responseWindow*whichFreq)
                if rsp(i) < ceil(imagesPerBlock*0.2)%if the key press is before the first possible target
