...
function [IsQuit, Performance] = test_abcdd_try
...
%     'endpoint6'
    ...
    for i = 1:numBlocks
        ...
        allList= round(imagesPerBlock/2):1:round(imagesPerBlock-responseWindow*whichFreq-1);
        targ_tmp = randsample(1:floor(length(allList)/totalTargNum),totalTargNum)*totalTargNum*2;
    ...
    elseif length(rsp)>=1
        for i=1:length(rsp)
            for j=0:ceil(responseWindow*whichFreq)
                if rsp(i) < round(imagesPerBlock/2)%if the key press is before the first possible target
