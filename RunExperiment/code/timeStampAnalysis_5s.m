...
clx
    ...
    ts_1hz = zeros(5,numfiles*32);
    ts_2hz = zeros(10,numfiles*32);
    ts_3hz = zeros(15,numfiles*32);
    ts_4hz = zeros(20,numfiles*32);
    ts_5hz = zeros(25,numfiles*32);
    ts_6hz = zeros(30,numfiles*32);
    ts_7hz = zeros(37,numfiles*32);
    ts_8hz = zeros(42,numfiles*32);
    
    for i = 1:numfiles*32
        if size(timeStamps{i},1)==5
            ts_1hz(:,i) = timeStamps{i}(:);
        elseif size(timeStamps{i},1)==10
            ts_2hz(:,i) = timeStamps{i}(:);
        elseif size(timeStamps{i},1)==15
            ts_3hz(:,i) = timeStamps{i}(:);
        elseif size(timeStamps{i},1)==20
            ts_4hz(:,i) = timeStamps{i}(:);
        elseif size(timeStamps{i},1)==25
            ts_5hz(:,i) = timeStamps{i}(:);
        elseif size(timeStamps{i},1)==30
            ts_6hz(:,i) = timeStamps{i}(:);
        elseif size(timeStamps{i},1)==37
            ts_7hz(:,i) = timeStamps{i}(:);
        elseif size(timeStamps{i},1)==42
