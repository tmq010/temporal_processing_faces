...
function [IsQuit, Performance] = test_abcdd_block_final(whichOrient)
...
%     PsychDebugWindowConfiguration(0, [,0.5]);
    ...
    for i = 1:numTrials
        ...
        % from 25 exemplars
        thelist = shuffle(shuffle(repmat(shuffle(1:1:itemsPerBlock),1,150),2),2);
    ...
    gray = 138;
    translayer = 2;
    mask = uint8(zeros(screenRect(3), screenRect(4), translayer));
    mask(:,:,1) = gray;
    ...
    for theTrial=1:numTrials
        ...
        for theitem = 1:itemsPerBlock
            ...
            if whichOrient==1
                ...
                offScrPtr(gender,theitem) = Screen('MakeTexture', window,  imgArray{gender,theitem});
            ...
            elseif whichOrient==2
                ...
                offScrPtr(gender,theitem) = Screen('MakeTexture', window,  flipdim(imgArray{gender,theitem},1));
            end
        ...
        for theFrame = 1:framesPerItem(theTrial)
            contrast = sin(a(theFrame)*pi); % levels of contrast
            mask(:,:,translayer) = uint8(round((1-contrast)*255));
            masktex(theFrame) = Screen('MakeTexture', window,  mask);
        end
        samples = [0.9:0.001:1.1];
        
        'endpoint7.1.2'
        %% start presenting images
        for i = 1:imagesPerTrial(theTrial)
            thepic = design{theTrial}(i,1);
            pixRect_scale = pixRect*randsample(samples,1);
            %scale image randomly between 90% and 110%
            for theFrame = 1:framesPerItem(theTrial)
                Screen('DrawTexture', window, offScrPtr(whichGender(theTrial),thepic),[], CenterRectOnPoint(pixRect_scale, screenRect(3)/2,screenRect(4)/2));
                Screen('DrawTexture', window, masktex(theFrame));
                %presStart = GetSecs;
...
%                 end
        ...
        for theitem = 1:itemsPerBlock
            Screen('Close', offScrPtr(gender,theitem));
        end
        for theFrame = 1:framesPerItem(theTrial)
            Screen('Close', masktex(theFrame));
        end
    end
    %         'endpoint9'
